...
def creatmodel():
    ...
    mo.add(layers.Conv2D(filters=20,strides=[4,4], kernel_size=[9, 9], activation='relu', padding='same', input_shape=[46, 48,3]))
    mo.add(layers.MaxPooling2D(pool_size=(2,2),strides=[2,2],padding='same'))
    mo.add(layers.Conv2D(filters=40,strides=[2,2], kernel_size=[4, 4], activation='relu', padding='same', input_shape=[46, 48,3]))
    mo.add(layers.MaxPooling2D(pool_size=(2,2),strides=[2,2],padding='same'))
...
model=creatmodel()
lena = mpimg.imread('C:\\Users\\fyx\\Desktop\\资料下载\\images\\_001a_0.png') # 读取和代码处于同一目录下的 lena.png
...
history = model.fit(partial_x_train, partial_y_train, epochs=300,batch_size=300,validation_data=(x_val, y_val))
#model.predict()
